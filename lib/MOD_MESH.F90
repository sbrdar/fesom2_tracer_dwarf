...
TYPE SPARSE_MATRIX
     ...
     real(kind=MP),  allocatable,   dimension(:) :: pr_values !preconditioner values
END TYPE SPARSE_MATRIX

TYPE T_MESH
integer                                     :: nod2D           ! the number of 2D nodes
real(kind=MP)                               :: ocean_area
real(kind=MP)                               :: ocean_areawithcav
real(kind=MP), allocatable, dimension(:,:)  :: coord_nod2D
real(kind=MP), allocatable, dimension(:,:)  :: geo_coord_nod2D
...
real(kind=MP), allocatable, dimension(:,:)  :: x_corners
real(kind=MP), allocatable, dimension(:,:)  :: y_corners ! cornes for the scalar points
integer,       allocatable, dimension(:)    :: nod_in_elem2D_num
real(kind=MP), allocatable, dimension(:)    :: depth                ! depth(n) is the depths at node n
real(kind=MP), allocatable, dimension(:,:)  :: gradient_vec
                                                           ! coefficients of linear reconstruction
                                                           ! of velocities on elements
real(kind=MP), allocatable, dimension(:,:)  :: gradient_sca ! Coefficients to compute gradient of scalars
                                                           ! on elements
...
real(kind=MP), allocatable, dimension(:)    :: zbar
real(kind=MP), allocatable, dimension(:)    :: Z
real(kind=MP), allocatable, dimension(:)    :: elem_depth
...
real(kind=MP), allocatable, dimension(:,:)  :: area
real(kind=MP), allocatable, dimension(:,:)  :: area_inv
real(kind=MP), allocatable, dimension(:,:)  :: areasvol
real(kind=MP), allocatable, dimension(:,:)  :: areasvol_inv
...
real(kind=MP), allocatable, dimension(:)    :: cavity_depth


real(kind=MP), allocatable, dimension(:,:)  :: cavity_nrst_cavlpnt_xyz
...
!#if defined (__oasis)
real(kind=MP), allocatable, dimension(:)    :: lump2d_south
real(kind=MP), allocatable, dimension(:)    :: lump2d_north
...
!------------------------------
real(kind=MP), allocatable,dimension(:,:)   :: hnode
real(kind=MP), allocatable,dimension(:,:)   :: hnode_new
real(kind=MP), allocatable,dimension(:,:)   :: zbar_3d_n
real(kind=MP), allocatable,dimension(:,:)   :: Z_3d_n
! LA 2023-01-31 add icebergs
real(kind=MP), allocatable,dimension(:,:)   :: Z_3d_n_ib
...
real(kind=MP), allocatable,dimension(:)     :: bottom_elem_thickness
real(kind=MP), allocatable,dimension(:)     :: bottom_node_thickness

! --> The increment of total fluid depth on elements. It is used to update the matrix
real(kind=MP), allocatable,dimension(:)     :: dhe

! --> hbar, hbar_old: correspond to the elevation, but on semi-integer time steps.
real(kind=MP), allocatable,dimension(:)     :: hbar
real(kind=MP), allocatable,dimension(:)     :: hbar_old

! --> auxiliary array to store depth of layers and depth of mid level due to changing
!     layer thinkness at every node
!real(kind=MP), allocatable,dimension(:)     :: zbar_n
!real(kind=MP), allocatable,dimension(:)     :: Z_n

! new bottom depth at node and element due to partial cells
real(kind=MP), allocatable,dimension(:)     :: zbar_n_bot
real(kind=MP), allocatable,dimension(:)     :: zbar_e_bot

! new depth of cavity-ocean interface at node and element due to partial cells
real(kind=MP), allocatable,dimension(:)     :: zbar_n_srf
real(kind=MP), allocatable,dimension(:)     :: zbar_e_srf
...
real(kind=MP), allocatable, dimension(:)    :: coriolis
real(kind=MP), allocatable, dimension(:)    :: coriolis_node
