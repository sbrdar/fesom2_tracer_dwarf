...
! updates the input tracer ttf
    subroutine adv_tra_vert_impl(dt, w, ttf, partit, mesh)
      ...
      real(kind=MP), intent(inout)       :: ttf(mesh%nl-1, partit%myDim_nod2D+partit%eDim_nod2D)
...
! flux is not multiplied with dt
    subroutine adv_tra_ver_upw1(w, ttf, partit, mesh, flux, o_init_zero)
      ...
      real(kind=WP), intent(in)         :: W  (mesh%nl,   partit%myDim_nod2D+partit%eDim_nod2D)
      real(kind=MP), intent(inout)      :: flux(mesh%nl,  partit%myDim_nod2D)
...
subroutine adv_tra_vert_impl(dt, w, ttf, partit, mesh)
    ...
    real(kind=MP), intent(inout)       :: ttf(mesh%nl-1, partit%myDim_nod2D+partit%eDim_nod2D)
    real(kind=WP), intent(in)          :: W  (mesh%nl,   partit%myDim_nod2D+partit%eDim_nod2D)
    real(kind=MP)                      :: a(mesh%nl), b(mesh%nl), c(mesh%nl), tr(mesh%nl)
    real(kind=MP)                      :: cp(mesh%nl), tp(mesh%nl)
    real(kind=MP)                      :: zbar_n(mesh%nl), z_n(mesh%nl-1)
    integer                            :: nz, n, nzmax, nzmin
    real(kind=MP)                      :: m, zinv, dz, v_adv
    real(kind=WP)                      :: dt_inv
...
!#include "associate_mesh_ass.h"
    ...
    dt_inv=1.0_WP/dt
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(a, b, c, tr, cp, tp, n, nz, nzmax, nzmin, m, zinv, dz, v_adv)
!$OMP DO
    ...
    do n=1,partit%myDim_nod2D
        ...
        zbar_n=0.0_MP
        Z_n=0.0_MP
        zbar_n(nzmax)=mesh%zbar_n_bot(n)
        Z_n(nzmax-1) =zbar_n(nzmax) + mesh%hnode_new(nzmax-1,n)/2.0_MP
        do nz=nzmax-1,nzmin+1,-1
            zbar_n(nz) = zbar_n(nz+1) + mesh%hnode_new(nz,n)
            Z_n(nz-1)  = zbar_n(nz)   + mesh%hnode_new(nz-1,n)/2.0_MP
        end do
        ...
        ! 1/dz(nz)
        zinv=1.0_MP*dt    ! no .../(mesh%zbar(1)-mesh%zbar(2)) because of  ALE
        ...
        b(nz)= mesh%hnode_new(nz,n)+W(nz, n)*v_adv
        ...
        do nz=nzmin+1, nzmax-2
            ...
            b(nz)=mesh%hnode_new(nz,n)+max(0._WP, W(nz, n))*v_adv
        ...
        !_______________________________________________________________________
        nz=nzmin
        dz=mesh%hnode_new(nz,n) ! It would be (mesh%zbar(nz)-mesh%zbar(nz+1)) if not ALE
    ...
    end do ! --> do n=1,partit%myDim_nod2D
!$OMP END DO
!$OMP BARRIER
!$OMP END PARALLEL
end subroutine adv_tra_vert_impl
...
subroutine adv_tra_vert_ppm(dt, w, ttf, partit, mesh, flux, o_init_zero)
    ...
    logical, optional                  :: o_init_zero
    logical                            :: l_init_zero
    real(kind=MP)                      :: tvert(mesh%nl)
    real(kind=WP)                      :: tv(mesh%nl), aL, aR, aj, x
    real(kind=MP)                      :: dzjm1, dzj, dzjp1, dzjp2, deltaj, deltajp1
    integer                            :: n, nz, nzmax, nzmin
...
!       tv(nzmax-1)=0.5_WP*(ttf(nzmax-2,n)+ttf(nzmax-1,n))
        ...
        do nz=nzmin+1, nzmax-3
            ...
            dzjm1    = mesh%hnode_new(nz-1,n)
            dzj      = mesh%hnode_new(nz  ,n)
            dzjp1    = mesh%hnode_new(nz+1,n)
            dzjp2    = mesh%hnode_new(nz+2,n)
            ! Be carefull here vertical operation have to be done on NEW vertical mesh !!!
        ...
        do nz=nzmin, nzmax-1
            ...
            if (W(nz,n)>0._WP) then
                ...
                tvert(nz  )=tvert(nz)*mesh%area(nz,n)*W(nz,n)
            end if
            ...
            if (W(nz+1,n)<0._WP) then
                ...
                tvert(nz+1)=tvert(nz+1)*mesh%area(nz+1,n)*W(nz+1,n)
            end if
        end do

        !_______________________________________________________________________
        ! Surface flux
        tvert(nzmin)= -tv(nzmin)*W(nzmin,n)*mesh%area(nzmin,n)
        ! Zero bottom flux
        tvert(nzmax)=0.0_MP
...
!$OMP DO
    do n=1, partit%myDim_nod2D
        ...
        !_______________________________________________________________________
        ! Surface flux
        tvert(nzmin)= -W(nzmin,n)*ttf(nzmin,n)*mesh%area(nzmin,n)

        !_______________________________________________________________________
        ! Zero bottom flux
        tvert(nzmax+1)=0.0_MP

        !_______________________________________________________________________
        ! Other levels
        do nz=nzmin+1, nzmax
            tv=0.5_WP*(ttf(nz-1,n)+ttf(nz,n))
            tvert(nz)= -tv*W(nz,n)*mesh%area(nz,n)
        end do
