...
! flux is not multiplied with dt
    subroutine adv_tra_hor_upw1(vel, ttf, partit, mesh, flux, o_init_zero)
      ...
      real(kind=MP), intent(inout)      :: flux(  mesh%nl-1, partit%myDim_edge2D)
...
! flux is not multiplied with dt
    subroutine adv_tra_hor_muscl(vel, ttf, partit, mesh, num_ord, flux, edge_up_dn_grad, nboundary_lay, o_init_zero)
      ...
      real(kind=MP), intent(in)         :: edge_up_dn_grad(4, mesh%nl-1, partit%myDim_edge2D)
      logical, optional                 :: o_init_zero
...
!$OMP DO
    ...
    do edge=1, partit%myDim_edge2D
        ...
        if(el(2)>0) then
            ...
            a=0.5_MP*(a+r_earth*mesh%elem_cos(el(2)))
        end if
    ...
    do edge=1, partit%myDim_edge2D
        ...
        do nz=nu1, nu12-1
           ...
           Tmean2=ttf(nz, enodes(2))- &
                  ...
                  mesh%edge_dxdy(2,edge)*r_earth*edge_up_dn_grad(4,nz,edge))/6.0_WP*c_lo(2)

           Tmean1=ttf(nz, enodes(1))+ &
                  (2.0_WP*(ttf(nz, enodes(2))-ttf(nz,enodes(1)))+ &
                  mesh%edge_dxdy(1,edge)*a*edge_up_dn_grad(1,nz,edge)+ &
                  mesh%edge_dxdy(2,edge)*r_earth*edge_up_dn_grad(3,nz,edge))/6.0_WP*c_lo(1)
